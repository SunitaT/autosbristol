...
{
    jQuery('#offer_send_friend .send_friend').bind('click', function()
    {
        var my_error = false;
        ...
        if (!my_error)
        {
			...
			var post_id = jQuery('#offer_send_friend #post_id').val();

            var datastring = 'action=tfuse_send_to_a_friend&email_from=' + email_from + '&email_send_to=' + email_send_to + '&message=' + message+ '&post_id=' + post_id;
...
{
    jQuery('#offer_contact #send_seller_message').bind('click', function()
    {
        var my_error = false;
