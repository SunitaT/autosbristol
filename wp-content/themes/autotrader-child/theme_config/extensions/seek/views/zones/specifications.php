<?php
    ...
    foreach($options as $id=>$val){
        ...
        {
            $mileage_number         = apply_filters( 'tfuse_mileage_number_format', number_format(intval($val['value'])) );
        ...
        elseif( $id =='seek_property_engine_power_kw' ){
            ...
            $power_kw               = apply_filters('tfuse_power_kw_number_format', number_format(intval($val['value'])));
        ...
        elseif( $id =='seek_property_engine_size' ){
            $result[$id]['name']    = __('Engine Size:', 'tfuse');
            
            $engine_size            = apply_filters('tfuse_engine_size_number_format', number_format(intval($val['value'])));
        ...
        elseif( $id == 'seek_property_engine_power_bhp'){
            ...
            $power_bhp          = apply_filters('tfuse_power_bhp_number_format', number_format(intval($val['value'])));
            $power_bhp_symbol   = __(' BHP', 'tfuse');
            $power_bhp_out      = tfuse_symbol_position($power_bhp, $power_bhp_symbol, 1, ' ', true);
            $power_bhp_out      = apply_filters('tfuse_power_bhp_symbol_position', $power_bhp_out, $power_bhp, $power_bhp_symbol);
        ...
        elseif( $id == 'seek_property_price'){
            ...
            $price_number_str   = apply_filters( 'tfuse_price_number_format', number_format(intval($val['value'])) );
        ...
        elseif( $id == 'seek_property_year'){
            $result[$id]['name'] = __('Year:', 'tfuse');
            $result[$id]['value'] = $val['value'];
            continue;
        }
        elseif( $id == 'seek_property_color'){
            $result[$id]['name'] = __('Exterior Color:', 'tfuse');
            $result[$id]['value'] = $val['value'];
            continue;
        }
        ...
        elseif($val['type']=='taxonomy')
        {
            ...
            if(sizeof($val['value'])){
                foreach($val['value'] as $tax){
                    ...
                    //elseif($tax->taxonomy=='vehicle_exterior_colors')
                    //$terms .= '<a href="'.get_term_link($tax->slug,'vehicle_exterior_colors').'">'.$tax->name .'</a>'. __(', ', 'tfuse');
                    elseif($tax->taxonomy=='vehicle_interior_colors')
                    $terms .= '<a href="'.get_term_link($tax->slug,'vehicle_interior_colors').'">'.$tax->name .'</a>'. __(', ', 'tfuse');
                    elseif($tax->taxonomy=='vehicle_body_style')
                    $terms .= '<a href="'.get_term_link($tax->slug,'vehicle_body_style').'">'.$tax->name .'</a>'. __(', ', 'tfuse');
                    elseif($tax->taxonomy=='vehicle_mechanicals')
                    $terms .= '<a href="'.get_term_link($tax->slug,'vehicle_mechanicals').'">'.$tax->name .'</a>'. __(', ', 'tfuse');
                    else
            ...
            elseif( $id == 'vehicle_gearboxes'){
                $result[$id]['name'] = __('Transmission:', 'tfuse');
            }
			elseif( $id == 'vehicle_colors'){
            $result[$id]['name'] = __('Exterior Color:', 'tfuse');
        }
        //    elseif( $id == 'vehicle_exterior_colors'){
        //        $result[$id]['name'] = __('Exterior Color:', 'tfuse');
        //    }
            elseif( $id == 'vehicle_interior_colors'){
                $result[$id]['name'] = __('Interior Color:', 'tfuse');
            }
            ...
            elseif( $id == 'vehicle_body_style'){
                $result[$id]['name'] = __('Body Style:', 'tfuse');
            }
            ...
            elseif( $id == 'vehicle_mechanicals'){
                $result[$id]['name'] = __('Mechanicals:', 'tfuse');
            }
            else $result[$id]['name'] = '';
