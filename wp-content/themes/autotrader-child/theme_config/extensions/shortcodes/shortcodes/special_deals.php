...
function tfuse_special_deals($atts, $content = null)
{
    ...
    $html .= '<div class="special_offers">
        ...
        foreach($posts_array as $vehicle){
            ...
            $engine_size=TF_SEEK_HELPER::get_post_option('property_engine_size', 0, $vehicle->ID);
			$color=TF_SEEK_HELPER::get_post_option('property_color', 0, $vehicle->ID);
            $fuel_out = apply_filters('tfuse_power_fuel_symbol_position', tfuse_symbol_position($fuel, $fuel_symbol, 1, ' ', true), $fuel, $fuel_symbol);

            $price_number_str   = apply_filters( 'tfuse_price_number_format', number_format($price,0,'', '.') );
            ...
            $html.='<div class="special_item">
                ...
                <div class="special_text">
                    ...
                    <div class="info_row"><span>'.__('ENGINE:','tfuse').'</span> '.$engine_size.'</div>
                    <div class="info_row"><span>'.__('COLOR:','tfuse').'</span> '. substr($color,0,10) .'</div>
                    <div class="info_row"><span>'.__('MILEAGE','tfuse').'</span> '.number_format(TF_SEEK_HELPER::get_post_option('property_mileage', 1, $vehicle->ID),0,'',',').'</div>
